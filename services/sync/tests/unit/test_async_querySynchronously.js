_("Make sure querySynchronously will synchronously fetch rows for a query asyncly");
...
function run_test() {
  ...
  let r0 = Async.querySynchronously(c("DELETE FROM moz_formhistory"));
  do_check_eq(r0, null);

  _("Make sure there's nothing there");
  let r1 = Async.querySynchronously(c("SELECT 1 FROM moz_formhistory"));
  do_check_eq(r1, null);

  _("Insert a row");
  let r2 = Async.querySynchronously(c("INSERT INTO moz_formhistory (fieldname, value) VALUES ('foo', 'bar')"));
  do_check_eq(r2, null);

  _("Request a known value for the one row");
  let r3 = Async.querySynchronously(c("SELECT 42 num FROM moz_formhistory"), ["num"]);
  do_check_eq(r3.length, 1);
  do_check_eq(r3[0].num, 42);

  _("Get multiple columns");
  let r4 = Async.querySynchronously(c("SELECT fieldname, value FROM moz_formhistory"), ["fieldname", "value"]);
  ...
  let r5 = Async.querySynchronously(c("SELECT fieldname, value FROM moz_formhistory"), ["value", "fieldname"]);
  ...
  let r6 = Async.querySynchronously(c("INSERT INTO moz_formhistory (fieldname, value) SELECT 'foo', 'baz' UNION SELECT 'more', 'values'"));
  do_check_eq(r6, null);

  _("Get multiple rows");
  let r7 = Async.querySynchronously(c("SELECT fieldname, value FROM moz_formhistory WHERE fieldname = 'foo'"), ["fieldname", "value"]);
  do_check_eq(r7.length, 2);
  do_check_eq(r7[0].fieldname, "foo");
  do_check_eq(r7[1].fieldname, "foo");

  _("Make sure updates work");
  let r8 = Async.querySynchronously(c("UPDATE moz_formhistory SET value = 'updated' WHERE fieldname = 'more'"));
  do_check_eq(r8, null);

  _("Get the updated");
  let r9 = Async.querySynchronously(c("SELECT value, fieldname FROM moz_formhistory WHERE fieldname = 'more'"), ["fieldname", "value"]);
  ...
  let r10 = Async.querySynchronously(c("SELECT value, fieldname FROM moz_formhistory"), ["fieldname"]);
  ...
  try {
    r11 = Async.querySynchronously(query);
  } catch(e) {
    except = e;
  }
  ...
  _("Cleaning up");
  Async.querySynchronously(c("DELETE FROM moz_formhistory"));
